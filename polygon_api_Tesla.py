...
# Initialize with empty dataframe, will populate if API succeeds
if "results" in Ticker_Overview_data:
    df_Ticker_Overview_data = pd.DataFrame([Ticker_Overview_data["results"]])
else:
    print(f" No ticker overview data for {TICKER}")
    df_Ticker_Overview_data = pd.DataFrame()
...
# Check if results exist, else use empty list
Custom_bars = Custom_bars_data.get("results", [])
df_Custom_bars_data = pd.DataFrame(Custom_bars)

if len(df_Custom_bars_data) > 0:
...
output_path = os.path.join(onedrive_dir, f"{sheet_prefix}.xlsx")  # "GOOGL" or "AAPL"
